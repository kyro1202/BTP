...
void getDistance(int s, vector<double> &dist, vector<double> &time, vector<pair<ii,pair<double,double> > > &par)
{
	bool relax = false;
	int n = G.size();
	for(int i = 0 ; i < n ; i++)
...
pair<double,int> getCS(vector<double> dist, vector<double> dist2, vector<double> time)
{
	...
	for(int i = 0 ; i < n ; i++)
	{
		...
		{
			double temp = dist[i] + dist2[i] + getNewEdgeWeight(-1.0*stations[i].ff,stations[i].ss.ff + max(stations[i].ss.ss - time[i],0.0));
			if(temp < cur_min)
			{
				cur_min = temp;
				index = i;
...
pair<pair<double,double>,vii> BFalgorithm()
{
	...
	getDistance(source, dist, time, par);

	vector<double> dist2;
	vector<double> time2;
	vector<pair<ii,pair<double,double> > > par2(n,mp(mp(-1,-1),mp(0.0,0.0)));
	getDistance(destination, dist2, time2, par2);

	pair<double,int> optimalCS = getCS(dist, dist2, time);
	
		
	if(optimalCS.ff >= dist[destination])
	{
		return buildPathWithoutCharging(source, destination, par);
	}
	else
	{
		return buildPathWithCharging(source, optimalCS.ss, destination, time, par, par2);
	}
}
